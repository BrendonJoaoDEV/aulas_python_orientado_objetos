...
while True:
    ...
    pasta = 'aula001_definicao_classe/gravacao_contas/'
    
    # VErificando se a pasta existe, se não, irá criá-la:
    os.makedirs(pasta, exist_ok=True)
    
    # Nome para o arquivo CSV para gravar as informações:
    arquivo = 'aula001_definicao_classe/gravacao_contas/contas.csv'
    
    # Escrever a lista de dicionários no arquivo CSV:
    with open(arquivo, mode='w', newline='') as arquivo_contas:
        fieldnames = ['numero_conta', 'nome_titular',
                      'saldo', 'agencia', 'tipo_conta']
        writer = csv.DictWriter(arquivo_contas, fieldnames=fieldnames, delimiter=';')
